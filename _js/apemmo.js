...
APE.MmoClient = new Class({
	...
	initialize: function(target){
		...
		Ge.preload('/demos/mmo/img/map.png', this.drawDesign.bind(this));
			...
			*/
			...
			for(var i in this.skills){
				var el = new Element('div', {
					'class':'spell',
					style:'background-image:url(\'/demos/mmo/img/icon_'+this.skills[i].id+'.png\')'
				});
	...
	addCreep: function(creep){
		var unit = this.addUnit(
			'/demos/mmo/img/creep'+creep.type+'.png',
			'creep_'+creep.id,
			creep.pos.x,
			creep.pos.y,
			creep.totalLife
	...
	redraw: function(tick){
		...
		if(!this.drawing){
			...
			this.ctx.drawImage(Ge.getPreloaded('/demos/mmo/img/map.png').img, Math.round(map.x), Math.round(map.y));
