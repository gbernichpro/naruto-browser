...
if(!isset($_GET['token'])){ echo "<script>self.location='index.php?p=home'</script>"; return; }
if(!isset($_GET['user'])){ echo "<script>self.location='index.php?p=home'</script>"; return; }
$token = antiinjection2($_GET['token']);
$user = antiinjection2($_GET['user']);
$sqlv = mysql_query("SELECT id, email FROM usuarios WHERE usuario='".$user."'");
$dbv = mysql_fetch_assoc($sqlv);
if(md5($dbv['id'])<>$token){ echo "<script>self.location='index.php?p=home'</script>"; return; }
