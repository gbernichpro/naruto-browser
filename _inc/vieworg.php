...
$sqlo = mysql_query("SELECT * FROM organizacoes WHERE id=".antiinjection($_GET['id']));
if(mysql_num_rows($sqlo)==0){ echo "<script>self.location='?p=home'</script>"; return; }
$dbo = mysql_fetch_assoc($sqlo);
$sqlm = mysql_query("SELECT m.*,u.usuario,u.nivel niveluser FROM membros m LEFT OUTER JOIN usuarios u ON m.usuarioid=u.id WHERE m.status='sim' AND m.orgid='".antiinjection($_GET['id'])."' ORDER BY posicao ASC, niveluser DESC");
...
<div class="box_middle">
	<table width="100%" cellpadding="0" cellspacing="0">
        ...
        <tr style="background:url(_img/gradient2.jpg) repeat-y;">
        	<td style="padding-right:20px;"><b>Guerras Vencidas:</b></td>
            <td><?php echo $dbo['war_v']; ?> Guerras</td>
        </tr>
		<tr>
        	<td style="padding-right:20px;"><b>Guerras Perdidas:</b></td>
        ...
        <td style="padding-right:20px;"><b>Reserva:</b></td>
            <td><b></b> <?php echo number_format($dbo['reserva'],2,',','.'); ?> yens</td>
        </tr>
    </table>
  	<div class="sep"></div>
