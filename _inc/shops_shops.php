...
$pega=antiinjection($_GET["categoria"]);
if($pega=="Calçado"){$pega="calcado";
}
if($pega==""){$pega="arma";}
if(($pega<>'arma')&&($pega<>'Vestimenta')&&($pega<>'calcado')&&($pega<>'bijuu')&&($pega<>'acessorios')){
echo "<script>self.location='?p=homse'</script>"; return; }
$sqls=mysql_query("SELECT id,nome,categoria FROM table_itens Where categoria='$pega' ORDER BY nome ASC");
...
<b>Selecione o Item:</b><br />
	...
	<select id="item" name="item">
		...
		switch($dbs['categoria']){
			case 'acessorios': $categoria='acessorios'; break;
			case 'arma': $categoria='arma'; break;
			case 'bijuu': $categoria='bijuu'; break;
