...
if(isset($_GET['buy'])){
	...
	if(mysql_num_rows($sqli)==0){ echo "<script>self.location='?p=viewshop&shop=".strtolower($_GET['shop'])."&msg=1'</script>"; return; }
	$dbi = mysql_fetch_assoc($sqli);
	$sqlbom=mysql_query("select usuario from usuarios where id='".antiinjection($dbi['usuarioid'])."'");
	$junrios=mysql_fetch_assoc($sqlbom);
	if($dbi['usuarioid']==$db['id']){ echo "<script>self.location='?p=viewshop&shop=".strtolower($_GET['shop'])."&msg=3'</script>"; return; }
	if($dbi['venda']=='nao'){ echo "<script>self.location='?p=home'</script>"; return; }
	if($db['yens']<$dbi['valor']){ echo "<script>self.location='?p=viewshop&shop=".strtolower($_GET['shop'])."&msg=2'</script>"; return; }
	mysql_query("UPDATE usuarios SET yens=yens-".$dbi['valor'].", compraloja=compraloja+1 WHERE id=".$db['id']);
	mysql_query("INSERT INTO logcompras (data, assunto, msg) VALUES ('".date('Y-m-d H:i:s')."','O usuario:".$db['usuario']."', 'Acabou de comprar o item <b>".$dbi['nome']."</b>, que estava anunciado na loja de ".$junrios['usuario'].", foi vendido por <b>".number_format($dbi['valor'],2,',','.')." yens</b>.')");
	...
	mysql_query("UPDATE inventario SET venda='nao', valor=0, usuarioid='".$db['id']."' WHERE id='".antiinjection($_GET['buy'])."'");
    mysql_query("UPDATE usuarios SET yens=yens-".$dbi['valor']." WHERE id='".$db['id']."'");
mysql_query("INSERT INTO vendas (usuarioid, valor)
VALUES ('".$dbi['usuarioid']."', '".$dbi['valor']."')");
	echo "<script>self.location='?p=viewshop&shop=".strtolower($_GET['shop'])."&msg=4'</script>";
}
if(!isset($_GET['shop'])){ echo "<script>self.location='?p=home'</script>"; return; }
$sqlv=mysql_query("SELECT id FROM usuarios WHERE usuario='".antiinjection($_GET['shop'])."'");
...
<img width="142" style="" src="_img/_detalhes/msg/53.png"></td><td valign="top"><br><br><br>
...
Abaixo estão os itens da loja de <?php echo ucfirst($_GET['shop']); ?>.
</b>
