...
$categoria = 'animais';
$dif = $db['nivel']+7;
$sqls = mysql_query("SELECT * FROM table_animais WHERE categoria='animais' AND reqnivel<$dif ORDER BY reqnivel ASC");
...
<table width="100%" cellpadding="0" cellspacing="1">
    ...
    <tr style="background:#161616;" onmouseover="style.background='#310000'" onmouseout="style.background='#161616'">
        ...
        <td align="center" width="20%">
        	<b>Nivel Mínimo</b><br />
            <span class="sub2"><?php echo $dbs['reqnivel']; ?> pontos</span><br /><br />
            <b>Valor Unitário</b><br />
            <span class="sub2"><?php echo number_format($dbs['valor'],2,',','.'); ?> yens</span><br /><br />
            <form method="post" action="?p=shopanimal" onsubmit="subm.value='Carregando...';subm.disabled=true;">
            ...
            <?php if($db['animal']==1){ echo '<span class="sub2">Você já possui um contrato de invocação.</span>';}else if($dbs['vip']=='nao'){ ?><input type="submit" id="subm" name="subm" class="botao" value="Comprar" /><?php } else { if(date('Y-m-d H:i:s')>=$db['vip']) echo '<span class="sub2">Exclusiva para VIP.</span>'; else { ?><input type="submit" id="subm" name="subm" class="botao" value="Comprar" /><?php }} ?>
            </form>
        </td>
  </tr>
