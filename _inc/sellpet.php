...
if(isset($_POST['id'], $_POST['senha'])){
    ...
    // Busca o pet e valida dono
    $stmt_i = mysqli_prepare($mysqli_link, "SELECT itemid FROM animais WHERE id=? AND usuarioid=?");
    ...
    // Busca valor do pet
    $stmt_v = mysqli_prepare($mysqli_link, "SELECT nome, valor FROM table_animais WHERE id=?");
    ...
    try {
        // Remove pet
        $stmt_del = mysqli_prepare($mysqli_link, "DELETE FROM animais WHERE id=? AND usuarioid=?");
        mysqli_stmt_bind_param($stmt_del, "ii", $id, $db['id']);
        mysqli_stmt_execute($stmt_del);

        if(mysqli_stmt_affected_rows($stmt_del) == 0){
            throw new Exception("Falha ao remover pet ou pet inexistente.");
        }
        ...
        // Gera Log (Opcional para pets, mantendo padrão de sellitem se necessário)
        $msg = "Usuário vendeu o pet: {$dbv['nome']} por " . number_format($valor_venda, 2, ',', '.') . " yens.";
        ...
        die("Erro ao processar venda de pet: " . $e->getMessage());
    }
    exit();
}
...
$id_get = (int)$_GET['id'];
$stmt_q = mysqli_prepare($mysqli_link, "SELECT i.id, i.status, i.upgrade, i.usuarioid, t.categoria, t.descricao, i.taijutsu, i.ninjutsu, i.genjutsu, t.nome, t.imagem, t.valor 
                                       FROM animais i 
                                       LEFT OUTER JOIN table_animais t ON i.itemid=t.id 
...
<div class="modern-card">
    <div class="modern-card-header">🐾 Venda de Pet</div>
    <div class="modern-card-body">
        ...
        <div style="background: rgba(255,165,0,0.1); border-left: 4px solid gold; padding: 15px; margin-bottom: 25px; border-radius: 4px;">
            <p style="margin: 0; font-size: 13px; color: #ddd;">
                Atenção Ninja! Vender seu companheiro Pet retornará apenas <b>50% do valor pago</b>. Esta ação não poderá ser desfeita.
            </p>
        </div>
        ...
        <div style="display: flex; gap: 20px; align-items: start; background: rgba(0,0,0,0.2); padding: 20px; border-radius: 8px; border: 1px solid #333; margin-bottom: 25px;">
            ...
            <div style="flex: 1;">
                ...
                <p style="font-size: 12px; color: #888; margin-bottom: 15px; line-height: 1.4;"><?php echo $dbi['descricao'] ?: 'Seu fiel companheiro de batalhas.'; ?></p>
                
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(120px, 1fr)); gap: 10px;">
                    <?php if($dbi['taijutsu'] > 0): ?>
                        <div style="font-size: 11px; color: #ccc;"><img src="_img/equipamentos/up.png" width="12" style="vertical-align: middle;"> Taijutsu: <span style="color: #5f5;">+<?php echo ($dbi['taijutsu']); ?></span></div>
                    <?php endif; ?>
                    <?php if($dbi['ninjutsu'] > 0): ?>
                        <div style="font-size: 11px; color: #ccc;"><img src="_img/equipamentos/up.png" width="12" style="vertical-align: middle;"> Ninjutsu: <span style="color: #5f5;">+<?php echo ($dbi['ninjutsu']); ?></span></div>
                    <?php endif; ?>
                    <?php if($dbi['genjutsu'] > 0): ?>
                        <div style="font-size: 11px; color: #ccc;"><img src="_img/equipamentos/up.png" width="12" style="vertical-align: middle;"> Genjutsu: <span style="color: #5f5;">+<?php echo ($dbi['genjutsu']); ?></span></div>
                    <?php endif; ?>
                </div>

                <div style="margin-top: 20px; padding-top: 15px; border-top: 1px solid #333;">
                    <div style="font-size: 11px; color: #666; text-transform: uppercase;">Reembolso da Vila:</div>
        ...
        <form method="post" action="?p=sellpet" style="max-width: 400px; margin: 0 auto; background: rgba(0,0,0,0.1); padding: 20px; border-radius: 8px; border: 1px solid #222;" onsubmit="var b=this.querySelector('input[type=submit]'); if(b) { b.value='Processando...'; b.disabled=true; }">
            ...
            <div style="text-align: center;">
                <input type="submit" class="modern-btn" style="width: 100%; padding: 12px; background: #822; color: #fff; font-weight: bold;" value="VENDER PET" />
                <a href="?p=inventory" style="display: block; margin-top: 15px; font-size: 11px; color: #666; text-decoration: none;">Cancelar e Sair</a>
            </div>
        </form>
