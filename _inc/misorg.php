...
if(!isset($_GET['id'])){ echo "<script>self.location='?p=myorg'</script>"; return; }
if(($db['orgmissao']>0)or($db['missao']>0)){ echo "<script>self.location='?p=myorg&msg=3'</script>"; return; }
$get=$c->decode($_GET['id'],$chaveuniversal);
$ex=explode(',',$get);
$sqlm = mysql_query("SELECT * FROM table_missoes WHERE id='".antiinjection($ex[0])."'");
if(mysql_num_rows($sqlm)==0){ echo "<script>self.location='?p=myorg&msg=2'</script>"; return; }
...
if($dbm['membros']==$dbm['maximo']){
	...
	mysql_query("UPDATE usuarios SET orgmissao=0, missao=".$dbm['id'].", missao_fim='".antiinjection($missaofim)."' WHERE orgmissao=".$dbm['id']);
