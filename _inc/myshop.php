...
if(isset($_GET['remove'])){
	vn($_GET['remove']);
	$sqli = mysql_query("SELECT usuarioid FROM inventario WHERE id='".antiinjection($_GET['remove'])."'");
	$dbi = mysql_fetch_assoc($sqli);
	if($dbi['usuarioid']<>$db['id']){ echo "<script>self.location='?p=home'</script>"; return; }
	mysql_query("UPDATE inventario SET venda='nao', valor=0 WHERE id='".antiinjection($_GET['remove'])."'");
...
<div class="box_middle">Abaixo estão os itens que você está vendendo em sua loja. Você pode utilizar esta página para remover os itens que não deseja mais vender. Caso venda algum item, o dinheiro ganho ficará guardado em sua loja, e será adicionado à sua reserva apenas quando preferir.
	...
	if(isset($_GET['msg'])){
		switch($_GET['msg']){
		    ...
		    case 5: $msg='Sua senha da minha loja está incorreta!'; break;
		}
