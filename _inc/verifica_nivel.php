...
if((isset($_SESSION['logado']))&&($db['exp']>=$db['expmax'])){
	do{
		...
		mysql_query("UPDATE usuarios SET nivel=nivel+1, exp=$difexp, expmax=$novaexpmax, yens=yens+300, yens_fat=yens_fat+300, energia=energia+100, energiamax=energiamax+100 , pontos=pontos +1 WHERE id=".$db['id']);
...
if((isset($_SESSION['logado']))&&($db['doujutsu_exp']>=$db['doujutsu_expmax'])&&($db['doujutsu_nivel']<30)){
	do{
		...
		if($db['doujutsu']==4) $doujutsu='Mangekyou_Sharingan'; else
