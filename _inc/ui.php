...
$sqli=mysql_query("SELECT u.id, u.usuario, u.yens, u.yens_fat, u.nivel, u.orgid, u.energia, u.energiamax, u.taijutsu, u.ninjutsu, u.genjutsu, u.personagem, u.avatar, u.renegado, u.vila, u.doujutsu, u.doujutsu_nivel, u.doujutsu_exp, u.doujutsu_expmax, u.exp, u.expmax, u.vip, u.missao, u.loginip, u.tipo, o.nivel orgnivel FROM usuarios u LEFT OUTER JOIN organizacoes o ON u.orgid=o.id WHERE u.id=".$_SESSION['prepare']);
...
if(($dbi['renegado']=='sim')&&($db['renegado']=='sim')){ echo "<script>self.location='?p=hunt&msg=11'</script>"; return; }
if(($dbi['vila']==$db['vila'])&&($dbi['renegado']=='nao')&&($db['renegado']=='nao')){ echo "<script>self.location='?p=hunt&msg=11'</script>"; return; }
...
if($db['doujutsu']==4){ $txtdoujutsu1='mangekyou_sharingan'; $addgen1=round($db['genjutsu']*($db['doujutsu_nivel']/50)); } else $addgen1=0;
if($db['orgnivel']>0){
...
if($dbi['doujutsu']==4){ $txtdoujutsu2='mangekyou_sharingan'; $addgen2=round($dbi['genjutsu']*($dbi['doujutsu_nivel']/50)); } else $addgen2=0;
if($dbi['orgnivel']>0){
...
<div class="box_middle">
   ...
   $ku=mysql_query("SELECT i.id, t.id petid, t.nome, t.taijutsu, t.ninjutsu, t.genjutsu, t.imagem FROM pets i LEFT OUTER JOIN table_pets t ON i.petid=t.id WHERE i.usuarioid=".$db['id']." AND i.status='on'");

while($kuinfo=mysql_fetch_assoc($ku)){
$danoo= $kuinfo['taijutsu'];
		...
		if($i%2){
			if($jutsu1<=$maxj1){
				...
				$dano=danojutsu(($db['ninjutsu']+$danoo),$dbi['genjutsu'],($forcajutsu1[$jutsu1]+(floor(($niveljutsu1[$jutsu1]-1)*5))));
