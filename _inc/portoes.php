...
if(isset($_GET['action'])){
	...
	$sqli=mysql_query("SELECT usuarioid,categoria FROM portao WHERE id=".$id);
	...
	vn($id);
	mysql_query("UPDATE portao SET status='off' WHERE usuarioid=".$db['id']." AND categoria='".$categoria."'");
	if($act=='on') mysql_query("UPDATE portao SET status='on' WHERE id=".$id);
	if($act=='on') echo "<script>self.location='?p=portao&msg=2'</script>"; else echo "<script>self.location='?p=portao&msg=3'</script>";
}



$sqli=mysql_query("SELECT i.id,i.status,i.upgrade,i.expira,t.categoria,t.descricao,t.porcentagem,t.ninjutsu,t.genjutsu,t.nome,t.imagem,t.valor FROM portao i LEFT OUTER JOIN table_portoes t ON i.itemid=t.id WHERE i.usuarioid=".$db['id']." AND venda='nao' ORDER BY status ASC");
$dbi=mysql_fetch_assoc($sqli);
?>
 ...
 <div style="background:url(_img/gradient.jpg) repeat-y;color:#FFFFAA;"><img src="_img/yens.png" width="14" height="14" /> <b>Meus Yens: <?php echo number_format($db['yens'],2,',','.'); ?> yens</b></div>
    ...
    <tr class="table_dados" style="background:#323232;" onmouseover="style.background='#2C2C2C'" onmouseout="style.background='#323232'">
        ...
        <td style="padding:5px;">
            ...
            <b><span class="sub2"><?php if($dbi['porcentagem']>0) echo '<img src="_img/equipamentos/up.png" width="14" height="14" align="absmiddle" /> Este portão te dará um aumento na batalha de '.$dbi['porcentagem'].' % em Taijutsu<br />'; ?></span>
            <br />
          ...
          <a href="?p=portao&action=<?php if($dbi['status']=='off') echo 'on'; else echo 'off'; ?>&id=<?php echo $dbi['id']; ?>"><?php if($dbi['status']=='off') echo 'Ativar Portão'; else echo 'Desativar Portão'; ?></a>
          </td>
  	</tr>
    ...
    <div class="sep"></div>
    <div class="aviso">Você não possui nenhum portão do chakra.<a href="?p=shoppportao">Adiquira um portão do chakra</a></div>
    <?php } ?>
</div></div>
