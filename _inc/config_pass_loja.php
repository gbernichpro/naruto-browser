...
mysql_query("UPDATE usuarios SET pass=1 WHERE id=".$db['id']);
            }
if(isset($_POST['pass_atual'])){
	...
	if($atual<>$db['pass']){ echo "<script>self.location='?p=config&type=pass_loja&msg=1'</script>"; return; }
	if($nova<>$nova2){ echo "<script>self.location='?p=config&type=pass_loja&msg=2'</script>"; return; }
	mysql_query("UPDATE usuarios SET pass='".$nova."' WHERE id=".$db['id']);
	echo "<script>self.location='?p=config&type=pass_loja&msg=3'</script>";
}
if(isset($_GET['msg'])){
	switch($_GET['msg']){
		...
		case 3: $msg='Senha alterada com sucesso agora pode usar na Minha loja!'; break;
	}
	echo '<div class="aviso">'.$msg.'</div><div class="sep"></div>';
}
if($db['pass']=="1"){
echo "<div class='aviso'>Sua senha é padrão '1' modifique imediatamente por segurança</div>";
            }
?>
<fieldset><legend>Alterar Senha(da Minha Loja)</legend>
	<form method="post" action="?p=config&amp;type=pass_loja" style="background:url(_img/config_pass.jpg) no-repeat right top;" onsubmit="subm.value='Carregando...';subm.disabled=true;">
        ...
        <div class="sep"></div>
        <span class="sub2">Caso tenha esquecido sua senha da minha loja basta <a href="?p=recoverteste">clicar aki</a>. e colocar seu email que será enviado a senha para caixa de entrada ou lixo.</span>
        <div class="sep"></div>
