...
if(isset($_GET['action'])){
	...
	$sqli=mysql_query("SELECT usuarioid,categoria='bolsas' FROM bolsas WHERE id=".$id);
	$dbi=mysql_fetch_assoc($sqli);
	if($dbi['usuarioid']<>$db['id']){ echo "<script>self.location='?p=home'</script>"; return; }
	$categoria = $dbi['categoria']='bolsas';
	$act = antiinjection($_GET['action']);
	vn($id);
	mysql_query("UPDATE bolsas SET status='off' WHERE usuarioid=".$db['id']." AND categoria='".$categoria."'");
	if($act=='on') mysql_query("UPDATE bolsas SET status='on' WHERE id=".$id);
...
if(isset($_GET['sell'])){
	...
	$sqli=mysql_query("SELECT itemid, usuarioid FROM bolsas WHERE id='".$id."'");
	$dbi=mysql_fetch_assoc($sqli);
	if($dbi['usuarioid']<>$db['id']){ echo "<script>self.location='?p=home'</script>"; return; }
	$data=date('Y-m-d H:i:s');
	$sqlv=mysql_query("SELECT valor FROM table_bolsas WHERE id=".$dbi['itemid']);
	$dbv=mysql_fetch_assoc($sqlv);
	mysql_query("DELETE FROM bolsas WHERE id=".$id);
	mysql_query("UPDATE usuarios SET yens=yens+".($dbv['valor'])." WHERE id=".$db['id']);
  echo "<script>self.location='?p=bolsas&msg=4&value=".($dbv['valor'])."'</script>";
}

$sqli=mysql_query("SELECT i.id,i.status,i.upgrade,t.categoria,t.descricao,t.taijutsu,t.ninjutsu,t.genjutsu,t.nome,t.imagem,t.valor FROM bolsas  i LEFT OUTER JOIN table_bolsas t ON i.itemid=t.id WHERE i.usuarioid=".$db['id']." AND venda='nao'   ORDER BY status ASC");
$dbi=mysql_fetch_assoc($sqli);
?>
...
vila, ou então anunciar em sua <a href="?p=myshop">própria loja</a>, para que outros jogadores o comprem.</br>
    ...
    <tr class="table_dados" style="background:#323232;" onmouseover="style.background='#2C2C2C'" onmouseout="style.background='#323232'">
        ...
        <td style="padding:5px;">
            ...
            <b>Valor de Venda</b>:
             ...
             <?php echo number_format(($dbi['valor']),0,',','.'); ?> yens<br />
           <a href="?p=bolsas&sell=<?php echo $dbi['id']; ?>">Vender</a>
          </td>
  	</tr>
    ...
    <div class="sep"></div>
    <div class="aviso">Nenhuma mochila em seu inventário.</div>
    <?php } ?>
</div></div>
