<?php
$sqlp = mysql_query("SELECT p.exp, p.expira, t.* FROM pets p LEFT OUTER JOIN table_pets t ON p.petid=t.id WHERE p.usuarioid='".$db['id']."'");
$dbp=mysql_fetch_assoc($sqlp);
if(mysql_num_rows($sqlp)>0){ ?>
<div class="box_top">Kuchiyose no Jutsu</div>
<div class="box_middle">Abaixo estão as informações do seu contrato de invocação.<div class="sep"></div>
    <table width="100%" cellpading="0" cellspacing="1" style="background:url(_img/skins/naruto/gradient5.jpg) repeat-x #161616;" onmouseover="style.background='url(_img/skins/naruto/gradient4.jpg) repeat-x #320000'" onmouseout="style.background='url(_img/skins/naruto/gradient5.jpg) repeat-x #161616'">
        <tr>
            <td width="220"><img src="_img/skins/naruto/kuchiyose/<?php echo $dbp['imagem']; ?>.png" /></td>
            <td>
            <?php
            $tai=$dbp['taijutsu'];
            $nin=$dbp['ninjutsu'];
            ...
            <div align="center" style="color:#FFFFAA;">
                <span style="margin-right:50px;"><b>Nível [<?php echo floor($dbp['exp']/100)+1; ?>]</b></span>
                <span><b>Experiência: </b> <?php echo $dbp['exp']%100; ?>/100</span>
            </div>
            <div class="sep2"></div>
            <div align="center" style="color:#FFFFAA;">
            <?php
			...
			$hora=explode(':',$ex[1]);
			?>
            <span><b>Expira em </b> <?php echo $data[2].'/'.$data[1].'/'.$data[0].', às '.$hora[0]; ?> horas</span>
            </div>
            </td>
        </tr>
    </table>
