...
$categoria = 'animais';
$dif = $db['nivel']+7;
$sqls = mysql_query("SELECT * FROM table_selos WHERE categoria='selos' AND reqnivel<$dif ORDER BY reqnivel ASC");
$dbs = mysql_fetch_assoc($sqls);
?>
<table width="100%" cellpadding="0" cellspacing="1">
    <?php if(mysql_num_rows($sqls)==0) echo '<tr><td colspan="3"><div class="aviso">Nenhum selo cadastrado.</div></td></tr>'; else do{ if(date('Y-m-d H:i:s')<$db['vip']) $dbs['valor']=$dbs['valor']-($dbs['valor']*0.2); ?>
    <tr style="background:#161616;" onmouseover="style.background='#310000'" onmouseout="style.background='#161616'">
        ...
        <td align="center" width="20%">
            ...
            <?php if($dbs['vip']=='nao'){ ?><input type="submit" id="subm" name="subm" class="botao" value="Adquirir" /><?php } else { if(date('Y-m-d H:i:s')>=$db['vip']) echo '<span class="sub2">Exclusiva para VIP.</span>'; else { ?><input type="submit" id="subm" name="subm" class="botao" value="Adquirir" /><?php }} ?>
            </form>
        </td>
  </tr>
