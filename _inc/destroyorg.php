...
if($dbv['liderid']<>$db['id']){ echo "<script>self.location='?p=home'</script>"; return; }
if(isset($_GET['token'])){
	$token = antiinjection($_GET['token']);
	if($token<>$db['senha']){ echo "<script>self.location='?p=home'</script>"; return; }
	mysql_query("DELETE FROM membros WHERE orgid=".$db['orgid']);
	mysql_query("DELETE FROM organizacoes WHERE id=".$db['orgid']);
	mysql_query("DELETE FROM clas_investimentos WHERE orgid=".$db['orgid']);
	if($db['missao']>=1000){
		mysql_query("UPDATE usuarios SET orgid='0', orgmissao=0, missao=0 WHERE orgid=".$db['orgid']);
	} else {
		mysql_query("UPDATE usuarios SET orgid='0', orgmissao=0 WHERE orgid=".$db['orgid']);
	}
