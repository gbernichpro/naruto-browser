...
if(isset($_POST['buy_id'])){
	...
	if($category<>'selos'){	$data=date('Y-m-d H:i:s');
	$usuario=$db['usuario'];
	$msg='Usuario tentou burlar o sistema de shop de selos mudando a categoria , o item não foi inserido na conta do usuario.';
	mysql_query("INSERT INTO log_bugs (data,usuario,msg)"."VALUES ('".$data."','".$usuario."','".$msg."')");
	echo "<script>self.location='?p=home'</script>"; return; }
	switch($category){
        case 'selos': $sqlb = mysql_query("SELECT valor, reqnivel, vip, vida FROM table_selos WHERE id='".$buy."'"); return;
	}
	...
	if($bloq>0){ echo "<script>self.location='?p=shopselos&category=".$page."&msg=".$bloq."'</script>"; return; }
	if($db['yens']<$valor){ echo "<script>self.location='?p=shopselos&category=".$page."&msg=1'</script>"; return; }
	$sqli = mysql_query("SELECT count(id) conta FROM selos WHERE usuarioid='".$db['id']."' AND itemid='".$buy."'");
    $dbi = mysql_fetch_assoc($sqli);
	if($dbi['conta']>0){ echo "<script>self.location='?p=shopselos&category=".$page."&msg=8'</script>"; return; }
	$sqli3 = mysql_query("SELECT count(id) conta FROM selos WHERE usuarioid=".$db['id']." AND itemid='".$buy."'");
    $dbi3=mysql_fetch_assoc($sqli3);
	if($dbi3['conta']>0){ echo "<script>self.location='?p=shopselos&category=".$page."&msg=8'</script>"; return; }
    $sqlbuscavida = mysql_query("select * from table_selos where id='".$buy."'");
	...
	mysql_query("INSERT INTO selos (usuarioid,itemid,categoria,expira) VALUES (".$db['id'].",".$buy.",'".$category."','".$expira."')");
...
&raquo; Selos!</b></div><br>
<div style="margin-left: 0px;margin-top: 35px;font-family: arial;font-size: 12px;color: #fff;"><b>
O selo amaldiçoado é uma habilidade dada por Orochimaru, que<br /> 
deseja utilizar as pessoas "contaminadas" por esse selo como seus </br>
subordinados, e assim tomar posse de seus corpos á fim de usá-los </br>
com seu jutsu de imortalidade.</br>
precisa para sua jornada no mundo ninja! Selecione uma das</br>
categorias abaixo e boas compras!</br>
...
<ul class="menu">
    ...
    <div class="sep"></div>
    <?php
	if(!isset($_GET['category'])) require_once('shopselos_selos.php'); else
    if(isset($_GET['category'])){
		switch($_GET['category']){
			case 'selos': require_once('shopselos_selos.php'); return;
		}
	}
	?>
</div></div>
