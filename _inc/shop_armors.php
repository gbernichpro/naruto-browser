...
$categoria = 'armors';
$dif = $db['nivel']+7;
// Conversão para mysqli - Categoria 'vestimenta'
$sqls = mysqli_query($mysqli_link, "SELECT * FROM table_itens WHERE categoria='vestimenta' AND reqnivel<$dif ORDER BY reqnivel ASC");
?>
<div class="shop-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 20px; padding: 10px;">
    <?php if(mysqli_num_rows($sqls)==0): ?>
        <div class="aviso" style="grid-column: 1 / -1;">Nenhuma vestimenta disponível para o seu nível.</div>
    <?php else: 
                ...
                <div style="display: flex; gap: 15px; margin-bottom: 15px;">
                    ...
                    <div style="flex: 1; font-size: 11px;">
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 5px;">
                            <span style="background: rgba(100,200,255,0.1); color: #aaf; padding: 2px 6px; border-radius: 4px;">DEFESA AUMENTADA</span>
                        </div>
                    </div>
                </div>
