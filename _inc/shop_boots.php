...
$categoria = 'boots';
$dif = $db['nivel']+7;
// Conversão para mysqli - Categoria 'calcado'
$sqls = mysqli_query($mysqli_link, "SELECT * FROM table_itens WHERE categoria='calcado' ORDER BY reqnivel ASC");
?>
<div class="shop-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 20px; padding: 10px;">
    <?php if(mysqli_num_rows($sqls)==0): ?>
        <div class="aviso" style="grid-column: 1 / -1;">Nenhum calçado disponível.</div>
    <?php else: 
    ...
    ?>
        <div class="modern-card" style="border: 1px solid rgba(255,255,255,0.05); background: rgba(0,0,0,0.4); display: flex; flex-direction: column;">
            <div class="modern-card-header" style="height: 45px; padding: 0 15px; font-size: 14px; display: flex; justify-content: space-between; align-items: center;">
                ...
                <span style="font-size: 10px; color: var(--text-dim);">Req: <?php echo $dbs['reqtai']; ?> Attr</span>
            </div>
                ...
                <div style="display: flex; gap: 15px; margin-bottom: 15px;">
                    ...
                    <div style="flex: 1; font-size: 11px;">
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 5px;">
                            <span style="background: rgba(200,200,100,0.1); color: #ff9; padding: 2px 6px; border-radius: 4px;">AGILIDADE</span>
                        </div>
                    </div>
                </div>
