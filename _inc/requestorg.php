...
$ex = explode(',',$get);
$id = antiinjection($ex[0]);
$minimo = antiinjection($ex[1]);
if($db['nivel']<$minimo){ echo "<script>self.location='?p=org&msg=4'</script>"; return; }
...
if($dbv['conta']>0){ echo "<script>self.location='?p=org&msg=3'</script>"; return; }
